...
def reset_timer():
    ...
    Label.config(timer_label, text="Timer", fg=GREEN)
    Label.config(check_marks, text="")
...
def start_timer():
    ...
    if reps == 1 or reps == 3 or reps == 5 or reps == 7:
        count_down(work_sec)
        Label.config(timer_label, text="Working", fg=GREEN)

    elif reps == 2 or reps == 4 or reps == 6:
        count_down(short_break_sec)
        Label.config(timer_label, text="Short Break", fg=PINK)

    else:
        count_down(long_break_sec)
        Label.config(timer_label, text="Long Break", fg=RED)
...
def count_down(count):
    ...
    if count > 0:
        ...
        for _ in range(work_section):
            marks += "✓"
        Label.config(check_marks, text=marks)
